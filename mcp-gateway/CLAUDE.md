...
| File | Issue | Fix |
|------|-------|-----|
| `src/services/auth_manager.py` | Blocking I/O in async function - `creds.refresh(Request())` blocks event loop | Used `asyncio.to_thread()` to run in thread pool |
| `src/services/rate_limiter.py` | `asyncio.sleep()` inside lock blocks other coroutines | Moved sleep outside lock, re-acquire after sleeping |
| `src/services/rate_limiter.py` | Daily quota reset at arbitrary time (24h from init) | Fixed to reset at UTC midnight for consistent behavior |

**Gotcha: Blocking I/O in async functions**
```python
# ❌ Wrong - blocks the event loop
async def _refresh_credentials(self, creds, ...):
    creds.refresh(Request())  # This is synchronous network I/O!
...
**Gotcha: Sleeping inside asyncio locks**
```python
# ❌ Wrong - blocks other coroutines waiting for the lock
async with self._locks[api]:
    if limit_exceeded:
        await asyncio.sleep(wait_seconds)  # Other coroutines blocked!
        # ... continue

# ✅ Correct - release lock before sleeping
async with self._locks[api]:
    if limit_exceeded:
        wait_seconds = calculate_wait()
        # Don't sleep here, release lock first

if wait_seconds > 0:
    await asyncio.sleep(wait_seconds)
    async with self._locks[api]:  # Re-acquire
        # ... continue
```

**Gotcha: UTC midnight reset for daily quotas**
```python
# ❌ Wrong - resets 24h after arbitrary initialization time
if now - limit.last_day_reset > timedelta(days=1):
    limit.current_day_count = 0
    limit.last_day_reset = now  # Arbitrary time!

# ✅ Correct - reset at consistent UTC midnight
def _get_utc_midnight(self) -> datetime:
    now = datetime.utcnow()
    return now.replace(hour=0, minute=0, second=0, microsecond=0)

if limit.last_day_reset < self._get_utc_midnight():
    limit.current_day_count = 0
    limit.last_day_reset = self._get_utc_midnight()
```
...
|----|------|---|-------|------|
| #997 | 11:49 AM | ✅ | MCP Gateway Mock Configuration Created | ~491 |
| #994 | " | 🔵 | MCP Gateway Environment Configuration Template | ~483 |
| #964 | 11:40 AM | 🔵 | MCP Gateway Python Dependencies and Configuration | ~568 |
| #963 | 11:39 AM | 🔵 | MCP Gateway Dockerfile Configuration | ~475 |
| #944 | 11:34 AM | 🔵 | MCP Gateway Architecture and Tool Definitions | ~539 |
</claude-mem-context>